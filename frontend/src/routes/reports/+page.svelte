<script lang="ts">
	...
	const scheduleTypeTriggerContent = $derived(
		selectedScheduleType === 'daily'
			? `Daily (Every day at ${String(selectedScheduleHour).padStart(2, '0')}:${String(selectedScheduleMinute).padStart(2, '0')})`
			: selectedScheduleType === 'weekly'
				? `Weekly (Every Monday at ${String(selectedScheduleHour).padStart(2, '0')}:${String(selectedScheduleMinute).padStart(2, '0')})`
	...
	function openFeedsForCategory(category: string) {
		goto(`/feeds?category=${encodeURIComponent(category)}`);
	}

	function openFeed(feedUrl: string) {
		goto(`/feeds?feed_url=${encodeURIComponent(feedUrl)}`);
	}
...
<Sidebar.Provider class="h-full">
	<div class="flex h-full w-full">
		...
		<FeedSidebar
			...
			onCategorySelect={openFeedsForCategory}
			onFeedSelect={openFeed}
				...
				<div class="px-4 py-4 sm:px-6 sm:py-6 md:px-8 md:py-8">
					...
					<div class="mb-6 flex flex-wrap items-center justify-between gap-3 md:mb-8">
						<div class="flex items-center gap-2">
							<Sidebar.Trigger />
							<h1 class="text-xl font-bold sm:text-2xl">Reports Dashboard</h1>
						</div>
					</div>
					...
					<div class="grid gap-6 md:gap-8">
						...
						<Card class="p-4 sm:p-6">
							...
							>
								<div>
									...
									<p class="text-sm text-muted-foreground">
										Manage automated daily and weekly reports
									</p>
								</div>
							...
							{#if schedules.length === 0}
								<div class="flex flex-col items-center justify-center py-12 text-center">
									<Clock class="mb-4 size-16 text-muted-foreground" />
										...
										<Table.Body>
											{#each schedules as schedule (schedule.id)}
												<Table.Row>
													...
													<Table.Cell>
														<Badge
															variant={schedule.schedule_type === 'daily' ? 'default' : 'secondary'}
														>
													...
													<Table.Cell class="font-mono text-sm">
														{String(schedule.schedule_hour).padStart(2, '0')}:{String(
													...
													<Table.Cell>
														<div class="flex items-center gap-2">
															...
															/>
															<span class="text-sm"
																>{schedule.enabled ? 'Enabled' : 'Disabled'}</span
															>
														</div>
													</Table.Cell>
						...
						<Card class="p-4 sm:p-6">
							...
							{#if files.length === 0}
								<div class="flex flex-col items-center justify-center py-12 text-center">
									<FileText class="mb-4 size-16 text-muted-foreground" />
									<p class="text-lg text-muted-foreground">No reports generated yet</p>
									<p class="text-sm text-muted-foreground">
										Generate reports from the Feeds page or wait for scheduled reports
									</p>
								</div>
							{:else}
								<div class="-mx-4 overflow-x-auto sm:mx-0">
...
<Dialog.Root bind:open={createDialogOpen}>
	<Dialog.Content class="max-h-[90vh] max-w-[95vw] overflow-y-auto sm:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description
				>Set up an automated schedule to generate reports for a category</Dialog.Description
			>
		</Dialog.Header>
		<div class="grid gap-4">
			...
			<div class="grid gap-2">
				...
				<Select.Root type="single" bind:value={selectedScheduleType}>
					...
					<Select.Content>
						<Select.Item value="daily" label="Daily">Daily (Every day)</Select.Item>
						<Select.Item value="weekly" label="Weekly">Weekly (Every Monday)</Select.Item>
					</Select.Content>
				</Select.Root>
			</div>
