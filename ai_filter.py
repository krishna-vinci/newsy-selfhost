...
def get_openai_client() -> AsyncOpenAI:
    ...
    if client is None:
        ...
        logger.info(f"AsyncOpenAI client initialized with base_url: {Config.OPENAI_BASE_URL}")
...
Does this article match the user's interests? Answer YES or NO only."""
        ...
        logger.debug(f"AI Filter - Title: {title[:50]}... | Match: {matches} | Response: {answer}")
        return matches
...
async def process_article_for_category(
    ...
    try:
        ...
        existing = await conn.fetchrow(
            "SELECT id FROM article_ai_matches WHERE article_id = $1 AND category_id = $2 AND prompt_hash = $3",
            article_id, category_id, prompt_hash
        )
        
        if existing:
            logger.debug(f"Article {article_id} already processed for category {category_id}")
            return True
        
        # Get article details
        article = await conn.fetchrow(
            "SELECT title, description, content FROM articles WHERE id = $1",
            article_id
        )
        ...
        if matches:
            ...
            await conn.execute(
                """
                INSERT INTO article_ai_matches (article_id, category_id, prompt_hash)
                VALUES ($1, $2, $3)
                ON CONFLICT (article_id, category_id) 
                DO UPDATE SET prompt_hash = EXCLUDED.prompt_hash, matched_at = CURRENT_TIMESTAMP
                """,
                article_id, category_id, prompt_hash
            )
            logger.info(f"Article {article_id} matched AI filter for category {category_id}")
            return True
        else:
            # Remove match if it exists (in case of prompt change)
            await conn.execute(
                "DELETE FROM article_ai_matches WHERE article_id = $1 AND category_id = $2",
                article_id, category_id
            )
            logger.debug(f"Article {article_id} did not match AI filter for category {category_id}")
            return False
            
    except Exception as e:
        logger.error(f"Error processing article {article_id} for category {category_id}: {e}")
        return False


async def process_new_article(article_id: int, category_name: str, conn: asyncpg.Connection):
    """
    ...
    """
    try:
        # Get category with AI filter enabled
        category = await conn.fetchrow(
            "SELECT id FROM categories WHERE name = $1 AND ai_enabled = true",
            category_name
        )
        ...
        await process_article_for_category(article_id, category_id, conn)
        
    except Exception as e:
        logger.error(f"Error in process_new_article for article {article_id}: {e}")


async def reprocess_category_articles(category_id: int, conn: asyncpg.Connection) -> Dict[str, int]:
    """
    ...
    try:
        ...
        category = await conn.fetchrow(
            "SELECT name, ai_prompt, ai_enabled FROM categories WHERE id = $1",
            category_id
        )
        ...
        await conn.execute(
            "DELETE FROM article_ai_matches WHERE category_id = $1",
            category_id
        )
        ...
        articles = await conn.fetch(
            "SELECT id FROM articles WHERE category = $1 ORDER BY id DESC",
            category_name
        )
        ...
        for article in articles:
            ...
            result = await process_article_for_category(article_id, category_id, conn)
            if result:
                matched += 1
        ...
        logger.info(f"Reprocessing complete for category {category_id}: {total} total, {matched} matched, {filtered} filtered")
