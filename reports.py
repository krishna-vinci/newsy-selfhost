    ...
    try:
        ...
        if report_type == "starred":
            query = """
                ...
                WHERE a.category = $1 AND a.starred = true AND f."isActive" = true
        ...
        if not articles:
            raise ValueError(f"No articles found for category '{category}' with report type '{report_type}'")
...
async def execute_scheduled_report(category: str, schedule_type: str):
    """Execute a scheduled report generation"""
    try:
        logging.info(f"Executing scheduled {schedule_type} report for category: {category}")
        ...
        logging.error(f"Error generating scheduled report for {category} ({schedule_type}): {e}")
...
async def generate_starred_report(category: str):
...
async def create_schedule(schedule: ReportSchedule):
    ...
    try:
        ...
        if not category_exists:
            raise HTTPException(status_code=400, detail=f"Category '{schedule.category}' does not exist")
        
        # Check for duplicate
        existing = await conn.fetchval(
            "SELECT id FROM report_schedules WHERE category = $1 AND schedule_type = $2",
            schedule.category, schedule.schedule_type
        )
        if existing:
            raise HTTPException(
                status_code=400,
...
async def update_schedule(schedule_id: int, schedule: ReportSchedule):
    """Update an existing schedule"""
    conn = await get_db_connection()
    try:
        # Check if schedule exists
        exists = await conn.fetchval(
            "SELECT EXISTS(SELECT 1 FROM report_schedules WHERE id = $1)",
            schedule_id
        )
        if not exists:
